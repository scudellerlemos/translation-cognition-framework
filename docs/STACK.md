...
| **nº2 — KB/lore** | `context_pack.select_kb()` (léxico) + `_load_kb_semantic()` (semântico, #169) → seções "5c." e "5d." do pacote | **léxico**: token do título da seção citado na cena. **semântico** (suplemento, nunca substitui): `embedder.search_kb()` sobre `kb_vectors`, dedupe contra o léxico. Ambos **gated pela mesma trava temporal de spoiler** (default-deny por reveal-por-seção) | ✅ **ligado e ativo** (#183) — todas as 11 seções de `universe_knowledge_base.md` (`translation_software`, universo `breath_of_fire_4`) agora têm `reveal` explícito: 3 não-narrativas (`Mecânicas de Combate e Stats`, `Itens e Equipamentos`, `Títulos e Tratamentos`) tageadas `reveal: safe`; as 8 seções narrativas (`Personagens — Grupo Principal`, `Personagens — NPCs e Antagonistas`, `Locais`, `Facções e Povos`, `Conceitos e Lore`, `Formas de Dragão de Ryu`, `Decisões Ratificadas`, `Decisões Abertas`) tageadas explicitamente `reveal: beyond_frontier` — mesmo efeito de runtime que ausência de tag (default-deny), mas documenta que foram revisadas e o reveal-por-cena foi deliberadamente adiado, não esquecido. `_load_kb_semantic()` confirmado retornando hits reais (não mockados) para as seções `safe` e continuando a bloquear conteúdo narrativo após a retag. **Bloqueio para reveal-por-cena real**: `scene_id` neste projeto é o identificador literal do arquivo de jogo (`AREAD###`/`AREAS###`, ver `projects/breath_of_fire_4/connector/table_schema.md`), sem nenhum mapeamento área→capítulo/enredo documentado no repo — sem esse mapa não é possível converter "spoiler vira seguro no capítulo X" em um `scene_id` real e verificável, e fabricar um número seria contra o princípio evidence-based do projeto (#184, #175). Candidato a issue nova: construir esse mapa área→capítulo antes de tentar reveal narrativo granular. Achado colateral: `tcf db index --kind kb` estava fora do `--kind` do CLI (choices só tinha `translation`/`decision`, mesmo `embedder.index_project` já suportando `kind="kb"` desde #169) — corrigido em `framework/cli.py` |
