...
| **nº2 — KB/lore** | `context_pack.select_kb()` (léxico) + `_load_kb_semantic()` (semântico, #169) → seções "5c." e "5d." do pacote | **léxico**: token do título da seção citado na cena. **semântico** (suplemento, nunca substitui): `embedder.search_kb()` sobre `kb_vectors`, dedupe contra o léxico. Ambos **gated pela mesma trava temporal de spoiler** (default-deny por reveal-por-seção) | ✅ **ligado e ativo** (#183) — todas as 11 seções de `universe_knowledge_base.md` (`translation_software`, universo `breath_of_fire_4`) têm `reveal` explícito. 3 não-narrativas tageadas `reveal: safe` (`Mecânicas de Combate e Stats`, `Itens e Equipamentos`, `Títulos e Tratamentos`). **1 seção com scene_id real, não estimado**: `Personagens — NPCs e Antagonistas` → `reveal: AREAD155`. Esse número veio de dado empírico, não chute: extraiu-se o diálogo real do jogo (`connector/extract.py` contra os `.DAT` legítimos do usuário, 6046 falas de 125 arquivos — `dialogs.csv`, gitignorado, nunca commitado) e minerou-se, pra cada entidade de `entities.csv`, a **primeira cena em que seu nome aparece literalmente no texto** (`artifacts/state/entity_first_scene.json`, esse sim commitado — só scene_id/contagem, sem texto de diálogo). Pra essa seção, todos os 15 membros (Elina, Yuna, Deis, Marlok, Rasso, Abbess, Chino e NPCs menores) foram encontrados; usou-se o **mais conservador** (Rasso, `AREAD155`) como gate da seção inteira, já que o gate é por seção e não por linha — abre só quando o membro mais tardio já foi mencionado, nunca antes. Verificado: cena `AREAD010` bloqueia a seção, `AREAD160` libera. **Rodada 2, mesmo método**: mais 4 seções resolvidas com dado real. `Facções e Povos` → `reveal: AREAS015` — todos os 7 membros fechados (inclusive `Eastern Alliance` e `Hesperia`, que na rodada 1 pareciam ausentes do corpus só porque a mineração buscava o nome canônico PT-BR exato; grep direto em inglês confirmou presença de ambos). `Conceitos e Lore` → `reveal: AREAD077` — aqui a mineração ingênua (primeira menção) teria vazado o twist: `Yorae Dragon` aparece cedo (`AREAS015`) como pergunta misteriosa ("Gods, you say? Yorae Dragon?"), então usou-se em vez disso a cena da confirmação textual real do twin-god reveal (deus fundido em duas metades), lida manualmente na lista cronológica de ocorrências de "Yorae" — evita a armadilha "primeira menção ≠ seguro revelar" identificada na rodada 1. `Decisões Ratificadas (2026-06-21)` → `reveal: AREAS036` — máximo conservador entre as entidades referenciadas (Endless, Hex, Abbess, Ryu, Elina/Deis), conteúdo de processo sem spoiler de enredo. `Decisões Abertas` → `reveal: safe` — nota pura de convenção de estilo (gaguejo do Scias), sem conteúdo de trama, segura independente do nome do Scias permanecer não-resolvido. Todos os 3 gates com scene_id verificados end-to-end (bloqueado antes, revelado depois). **Rodada 3 — tabela complementar (`connector/mine_scene_order.py`, #183):** os 2 casos restantes que dependiam só de cobertura de corpus foram fechados ampliando a família DAT escaneada. Scan das 972 DAT do jogo (todas as famílias, não só `AREAD`/`AREAS`) confirmou que `AREAE*` (eventos de área) e `AREAM*` (mapas) — ambas marcadas "fase futura" só pra **tradução** — contêm texto narrativo real: falas de NPC com `[14][XX]@` em `AREAE` (`"Did you come from Worent?"`) e definições de local em `AREAM` (`"It's called Pung'tap, which means The Tower of Wind."`, `"You are now able to draw upon the Rock Dragon's power!"`). Criou-se `connector/mine_scene_order.py` — reusa os mesmos parsers binários de `extract.py` (TOC/seção-de-texto/strings, mesmo container em todas as famílias) mas amplia o escopo pra `{AREAD, AREAS, AREAE, AREAM}` só pra fins de ordem narrativa; **não** expande o escopo de tradução (`dialogs.csv` continua só `AREAD`/`AREAS`, decisão intencional documentada em `table_schema.md`). Regenerou `entity_first_scene.json` (79 entidades, 72 com 1ª cena encontrada — subiu de 64). `Locais` → `reveal: AREAM037` — todos os 23 membros fechados (inclusive `Worent`/`Pung'tap`, que só existiam em `AREAE`/`AREAM`; e `Hesperia`, que nem estava em `entities.csv` — adicionada com citação real do corpus). `Formas de Dragão de Ryu` → `reveal: AREAE038` — todos os 5 membros fechados (`Sand`/`Rock`/`Tree Dragon` só existiam em `AREAE`/`AREAM`, não em texto de item como se suspeitava). Ambos gates verificados end-to-end (bloqueado antes, revelado depois), e regressão checada nos 4 gates das rodadas 1/2 após re-migração do DB — todos seguem corretos. **1 seção permanece `reveal: beyond_frontier`, agora confirmada estruturalmente irresolúvel por este método**: `Personagens — Grupo Principal` (Ryu/Nina/Cray/Scias/Ursula/Ershin) — 0 ocorrências literais em **toda** a árvore de 972 DAT do jogo (não só nas famílias narrativas), só existe via control code `[04][XX]` de nome-variável; `artifacts/state/portrait_codes.json` proíbe explicitamente inferir esse mapeamento código→nome por heurística de texto, e não há tabela real (não-heurística) disponível pra esse mapeamento. Resolver isso exigiria engenharia reversa do formato binário de nome-variável — fora do escopo deste método de mineração textual. `_load_kb_semantic()` confirmado retornando hits reais pras 7 seções agora resolvidas (4 `safe` + `NPCs e Antagonistas` + `Facções e Povos` + `Conceitos e Lore` + `Decisões Ratificadas` + `Locais` + `Formas de Dragão de Ryu` = 10 de 11) e continuando a bloquear só `Grupo Principal`. Achado colateral: `tcf db index --kind kb` estava fora do `--kind` do CLI (choices só tinha `translation`/`decision`, mesmo `embedder.index_project` já suportando `kind="kb"` desde #169) — corrigido em `framework/cli.py`. Candidato a issue nova: `reveal` granular por entidade/linha (não só por seção) — não é mais bloqueador prático (só resta 1 seção sem gate real), mas destravaria per-linha em vez de conservador-máximo-por-seção pras seções já resolvidas |
