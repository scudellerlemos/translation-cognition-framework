...
| **nº2 — KB/lore** | `context_pack.select_kb()` (léxico) + `_load_kb_semantic()` (semântico, #169) → seções "5c." e "5d." do pacote | **léxico**: token do título da seção citado na cena. **semântico** (suplemento, nunca substitui): `embedder.search_kb()` sobre `kb_vectors`, dedupe contra o léxico. Ambos **gated pela mesma trava temporal de spoiler** (default-deny por reveal-por-seção) | ✅ **ligado e ativo** (#183) — 3 seções não-narrativas (`Mecânicas de Combate e Stats`, `Itens e Equipamentos`, `Títulos e Tratamentos`) tageadas `reveal: safe` em `translation_software` (universo de `breath_of_fire_4`); `_load_kb_semantic()` confirmado retornando hits reais (não mockados) para cena de teste. Seções narrativas (personagens/locais/facções/lore) seguem sem `reveal` — default-deny — até curadoria de spoiler por cena. Achado colateral: `tcf db index --kind kb` estava fora do `--kind` do CLI (choices só tinha `translation`/`decision`, mesmo `embedder.index_project` já suportando `kind="kb"` desde #169) — corrigido em `framework/cli.py` |
