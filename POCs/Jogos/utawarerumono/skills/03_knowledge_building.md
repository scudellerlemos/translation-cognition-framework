...
Varrer `dialogs.csv` e `entities_candidates.json` e extrair:

- **Definições in-game:** linhas que explicam um termo diretamente (ex: "O Akuruka é o poder que...")
...
Anotar cada achado com o offset de origem para citação posterior.
...
| Versões em outros idiomas | `"[Título] [ES/FR/DE] translation"` — referência para decisões terminológicas |
...
🚨 **Crítico:** wikis de jogos contêm spoilers de late-game. Definir antes de ler qualquer fonte:
...
Exemplo: para Utawarerumono Cap. 1–5, ler apenas a seção de "Personagens iniciais" da wiki. Não ler "Identidades reveladas", "Final do jogo", ou qualquer seção de arco posterior.
...
- Entrevistas com desenvolvedores em fontes fechadas
...
|------|------|----------|-----------|
| 1 — Canônico | Primeira mão, definitivo | Texto do jogo (dialogs.csv), guia oficial, site do desenvolvedor, artbook oficial | Usar diretamente; conflitos resolvem em favor do Tier 1 |
| 2 — Verificado | Comunitário com evidências | Wiki de fandom bem mantida com citações, tradução oficial em outro idioma, FAQ de speedrunner detalhado | Usar com verificação cruzada; preferir quando corroborado por múltiplas entradas |
...
#### 4. Contexto de uso
Onde e como a entidade aparece. Quais offsets no corpus confirmam.
...
**Fontes:**
- SRC-001 (Fandom Wiki): seção "[nome da seção]"
- SRC-002 (dialogs.csv): offset 0x33cd
...
| `universe_knowledge_base.md` | Uma entrada por entidade relevante, cada afirmação citando ID de fonte |
...
|----|-------|------|------|----------------------|-------------|----------------|-------|-------|
| SRC-001 | Utawarerumono Fandom Wiki | Wiki | 2 | Caps. 1–10 (parcial) | [URL] | IA + Humano | Sim | Seções de arco final não lidas |
| SRC-002 | dialogs.csv | Corpus | 1 | Corpus completo | local | IA | Sim | Fonte primária |
| SRC-003 | GameFAQs FAQ | Guia | 2 | Caps. 1–15 | [URL] | Humano | Sim | Detalha mecânicas |
...
|-------|-----------|---------------|---------|-------|
| Akuruka | "sistema de poder" | "espírito guardião" | Humano | SRC-003 cita guia oficial |
