"""kernel.py — fachada fina de orquestracao (B3, "Evolucao do Motor").

NAO reimplementa nada: `run_scene`/`run_chapter`/`run_game` (todos ja tipados, sem dependencia de
Claude/MCP) + `validate_project` (Validation, B1) + `write_pack` (Memory/context_pack, B2 -- ja usa
...
B4 (Skill DSL) nao precisou de codigo novo: `framework/skills/skill_base.py` (Skill ABC) +
