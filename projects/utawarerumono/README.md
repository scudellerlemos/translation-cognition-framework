...
## Estado: pipeline 00→08 rodado em escala ✅ (2 cenas / 1025 linhas)
...
| **Gate de round-trip** | ✅ byte-a-byte — travado por **teste automatizado** `connector/test_roundtrip.py` (pytest, 7 testes) |
...
| **Passo 08 — reinserção** | `reinsert.py` → **`output/ScriptEvent.sdat`** + `.ips`. Tiers: T1=595, REPOINT_head=425, REPOINT_cont=5, **resíduo T4=0** |
...
Destaques: **modelo de ponteiro é FILE-RELATIVO** (não absoluto — correção crítica); `space_strategy`
→ `repoint` por run (`novo_offset − file_start`); tradução aplicada por **arquivo aprovado** + script
(a IA não edita dados à mão).

**Saída final:** [`output/ScriptEvent.sdat`](output/) + patch `.ips` — mesmo nome/extensão do input, pronto para repatch.

> ⏳ **Pendente — gate in-game (só o usuário):** confirmar que strings relocadas ao fim do arquivo
> exibem no jogo. Se falhar → Plano B (relocar dentro do arquivo + reescrever a tabela Pack).
...
`ScriptEvent.sdat` totalmente mapeado** (header `Filename`/`Pack`, 353 scripts; ponteiros `50 00`
**file-relativos**); `extract.py`/`reinsert.py` escritos e compartilhando `sdat_format.py`; round-trip
e repoint travados por `connector/test_roundtrip.py` (pytest). O bloco `connector` do `project.json`
declara a config; a entrega final é um patch IPS.
